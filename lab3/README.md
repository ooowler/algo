...
| Индекс | Параметры | Влияние |
|--------|-----------|---------|
| **LSH** | `nbits` | Больше бит → выше recall и размер, дольше поиск |
| **HNSW** | `M`, `efConstruction`, `efSearch` | `efSearch` двигает recall/latency без перестроения; больший `M`/`efConstruction` — тяжелее build и индекс |
| **IVF+PQ** | `nlist`, `m`, `nprobe` | `nprobe` ↑ → recall и latency; `nlist`/`m` влияют на качество PQ и размер |

### Итог по критериям задания

1. **Скорость индексации** — смотри столбец «Построение» в таблицах выше и на графике build-time (LSH обычно быстрее HNSW на 1M; IVFPQ — train+add).
2. **Размер индекса** — столбец «МБ»; LSH и PQ дают компактные коды, HNSW — граф + векторы.
3. **Полнота (recall)** — при сопоставимой задержке сравни recall@100 на рис. 1 и 5.

**Практический вывод:** на текущем full-run **HNSW** — единственное семейство, которое доходит примерно до **0.995 recall@100**, но платит за это большим индексом и долгим build. **IVF+PQ** даёт лучший компромисс по размеру, времени построения и latency; по эвристическому score лидирует `IVFPQ nlist=1024 m=16 nprobe=8`. **LSH** остаётся быстрым и простым baseline, но по полноте заметно уступает двум другим подходам.
